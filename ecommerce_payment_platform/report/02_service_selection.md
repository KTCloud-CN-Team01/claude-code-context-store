# 02 — 서비스 선정 근거 및 6개사 기술스택 상세 프로파일
...
> 본 문서는 (1) 선정 근거, (2) 5축(아키텍처/CI·CD/관찰성/복원력/서비스 메시) 기술스택 상세 프로파일 두 파트로 구성.
...
- 출처: [Benchmarking Istio & Linkerd CPU - Michael Kipper](https://medium.com/@michael_87395/benchmarking-istio-linkerd-cpu-c36287e32781) — ⚠️ 이 출처는 [08_pain_point_validation.md](./08_pain_point_validation.md) 재확인 결과 컨퍼런스 발표가 아닌 **블로그 포스트**.
...
- **카오스 엔지니어링 자료 풍부히 확인됨**: 자체 CI/CD 플랫폼 **Concord + Gremlin** 연동으로 카오스 실험 자동화. "카오스 엔지니어링"보다 **"리질리언스 엔지니어링"** 용어 선호, 팀 단위 **성숙도 레벨(1~3단계)** 체계 운영(발표 시점 50개+ 팀이 레벨2 통과). 자체 진단 도구 **"Resiliency Doctor"** 개발. Chaos Conf 2018에서 전 Netflix/Comcast 출신 엔지니어가 "Practicing Chaos Engineering at Walmart" 발표.
...
- **MSA 단독 정답론에 대한 반례 2건 확보**: Shopify(모듈러 모놀리스+Pod 샤딩), 당근페이(MSA 대신 Clean Architecture 모노레포) — 차별화 전략([05_differentiation.md](./05_differentiation.md)) 수립 시 "무조건 잘게 쪼개는 MSA"가 아닌 "도메인 특성에 맞는 경계 설계"를 핵심 메시지로 활용 가능.
...
- **카오스 엔지니어링 성숙도**: Alibaba(ChaosBlade, CNCF Sandbox 등재) ≈ 11번가(Chaos Mesh+ToxiProxy, 실제 OSS 컨트리뷰션까지) ≈ Walmart(Concord+Gremlin, 성숙도 레벨체계) > Shopify(BFCM 연계 정례화) > 당근마켓(사후 대응형, 워밍업 시스템) > 카카오페이(순수 카오스 미확인).
- **국내 기업 공통 특징**: 해외 대비 공개 포스트모템/장애 보고서 문화가 약함(11번가·카카오페이는 거의 비공개, 당근마켓만 예외적으로 투명한 장애 사례 공개) — 이는 [04_pain_points.md](./04_pain_points.md) 단계에서 "신뢰성 공시 부재"를 페인포인트로 다룰 근거가 됨.

## 다음 단계

- [x] [03_comparison_matrix.md](./03_comparison_matrix.md): 비교 매트릭스 + 강점/약점
